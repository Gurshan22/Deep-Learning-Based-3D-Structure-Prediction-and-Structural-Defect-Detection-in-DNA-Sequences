...
Original file is located at
    https://colab.research.google.com/drive/1_67ozKLI9fY9X21UvP3QNMdPeT2l1cuF
"""
...
class DNADeepLearningAnalyzer:
    ...
    def calculate_comprehensive_accuracy(self):
        ...
        print("\n3. OVERALL DEEP LEARNING SYSTEM PERFORMANCE")
        print("-" * 60)

        # Best performing models
        best_structure_model = max(overall_metrics['structure_models'].items(),
                                 key=lambda x: x[1]['accuracy'])
        best_defect_model = max(overall_metrics['defect_models'].items(),
                              key=lambda x: x[1]['accuracy'])

        # Combined system accuracy
        best_struct_acc = best_structure_model[1]['accuracy']
        best_defect_acc = best_defect_model[1]['accuracy']
        combined_accuracy = (best_struct_acc + best_defect_acc) / 2

        # System-wide metrics
        all_accuracies = [metrics['accuracy'] for metrics in overall_metrics['structure_models'].values()] + \
                        [metrics['accuracy'] for metrics in overall_metrics['defect_models'].values()]

        all_f1_scores = [metrics['f1'] for metrics in overall_metrics['structure_models'].values()] + \
                       [metrics['f1'] for metrics in overall_metrics['defect_models'].values()]

        all_roc_aucs = [metrics['roc_auc'] for metrics in overall_metrics['structure_models'].values()] + \
                      [metrics['roc_auc'] for metrics in overall_metrics['defect_models'].values()]

        overall_metrics['system_overall'] = {
            'best_structure_model': best_structure_model[0],
            'best_defect_model': best_defect_model[0],
            'best_structure_accuracy': best_struct_acc,
            'best_defect_accuracy': best_defect_acc,
            'combined_accuracy': combined_accuracy,
            'mean_accuracy': np.mean(all_accuracies),
            'mean_f1': np.mean(all_f1_scores),
            'mean_roc_auc': np.mean(all_roc_aucs),
            'accuracy_std': np.std(all_accuracies),
            'system_reliability': min(all_accuracies)
        }

        print(f"\nBest Structure Prediction Model: {best_structure_model[0].replace('_', ' ').title()}")
        print(f"  Accuracy: {best_struct_acc:.4f} ({best_struct_acc*100:.2f}%)")

        print(f"\nBest Defect Prediction Model: {best_defect_model[0].replace('_', ' ').title()}")
        print(f"  Accuracy: {best_defect_acc:.4f} ({best_defect_acc*100:.2f}%)")

        print(f"\nDEEP LEARNING SYSTEM-WIDE PERFORMANCE METRICS:")
        print(f"  Combined System Accuracy: {combined_accuracy:.4f} ({combined_accuracy*100:.2f}%)")
        print(f"  Mean Model Accuracy: {np.mean(all_accuracies):.4f} ({np.mean(all_accuracies)*100:.2f}%)")
        print(f"  Mean F1 Score: {np.mean(all_f1_scores):.4f}")
        print(f"  Mean ROC AUC: {np.mean(all_roc_aucs):.4f}")
        print(f"  Accuracy Standard Deviation: {np.std(all_accuracies):.4f}")
        print(f"  System Reliability (Min Accuracy): {min(all_accuracies):.4f} ({min(all_accuracies)*100:.2f}%)")
    ...
    def create_enhanced_double_helix(self, sequence, structure_type='Normal', defect_present=False):
        """Create highly detailed interactive 3D DNA double helix structure - Same as original"""
        n_bases = len(sequence)

        # DNA structural parameters (realistic values)
        rise_per_bp = 3.4  # Angstroms
        twist_per_bp = 36  # degrees
        major_groove_width = 22  # Angstroms
        minor_groove_width = 12  # Angstroms
        helix_radius = 10  # Angstroms

        # Generate helix coordinates
        theta = np.linspace(0, (n_bases * twist_per_bp * np.pi / 180), n_bases)
        z = np.linspace(0, n_bases * rise_per_bp, n_bases)

        # Strand 1 coordinates (5' to 3')
        x1 = helix_radius * np.cos(theta)
        y1 = helix_radius * np.sin(theta)
        z1 = z

        # Strand 2 coordinates (3' to 5', antiparallel)
        x2 = helix_radius * np.cos(theta + np.pi)
        y2 = helix_radius * np.sin(theta + np.pi)
        z2 = z

        # Add structural distortions based on prediction
        if structure_type == 'Distorted':
            # Add realistic distortions
            distortion_amplitude = 2.0
            distortion_freq = 2 * np.pi / 6  # Distortion every 6 base pairs

            x1 += distortion_amplitude * np.sin(distortion_freq * np.arange(n_bases))
            y1 += distortion_amplitude * np.cos(distortion_freq * np.arange(n_bases))
            x2 += distortion_amplitude * np.sin(distortion_freq * np.arange(n_bases) + np.pi)
            y2 += distortion_amplitude * np.cos(distortion_freq * np.arange(n_bases) + np.pi)

            # Add twist distortions
            theta += 0.2 * np.sin(2 * np.pi * np.arange(n_bases) / 4)

        # Add defect-specific alterations
        if defect_present:
            # Simulate bulges, kinks, or breaks
            defect_position = n_bases // 3
            if defect_position < len(x1):
                x1[defect_position] += 3
                y1[defect_position] += 3
                x2[defect_position] -= 2
                y2[defect_position] -= 2

        # Create complementary sequence
        complement = {'A': 'T', 'T': 'A', 'G': 'C', 'C': 'G'}
        comp_sequence = ''.join([complement.get(base, 'N') for base in sequence])

        # Enhanced color mapping for bases (more realistic colors)
        colors = {
            'A': '#FF6B6B',  # Red - Adenine
            'T': '#4ECDC4',  # Teal - Thymine
            'G': '#45B7D1',  # Blue - Guanine
            'C': '#FFA07A',  # Orange - Cytosine
        ...
        # Add nucleotide bases for strand 1
        for i, base in enumerate(sequence):
            fig.add_trace(go.Scatter3d(
                x=[x1[i]], y=[y1[i]], z=[z1[i]],
                mode='markers+text',
                marker=dict(size=12, color=colors[base],
        ...
        # Add nucleotide bases for strand 2
        for i, base in enumerate(comp_sequence):
            fig.add_trace(go.Scatter3d(
                ...
                marker=dict(size=12, color=colors[base],
                           line=dict(width=2, color='black'),
                           opacity=0.8),
                text=f'{base}({n_bases-i})', # Reverse indexing for antiparallel strand
                textposition="middle center",
                textfont=dict(size=10, color='white'),
                name=f'Strand 2',
                showlegend=False if i > 0 else True
        ...
        # Add base pair connections (simplified)
        ...
        fig.update_layout(
            title=f'Interactive 3D DNA Double Helix Model: {structure_type} Structure' + (' with Defect' if defect_present else ''),
            scene=dict(
                xaxis_title='X (Å)',
                yaxis_title='Y (Å)',
                zaxis_title='Z (Å)',
    ...
    def run_interactive_analysis(self):
        ...
        while True:
            ...
            if 'hybrid_structure' in self.models and 'hybrid_defect' in self.models:
                ...
                self.create_enhanced_double_helix(dna_sequence, structure_type=structure_prediction, defect_present=defect_prediction == 'Present')
    ...
    def run_pipeline(self):
        ...
        print("Starting DNA Deep Learning Analysis Pipeline...")
        ...
        # 4. Calculate comprehensive accuracy metrics
        accuracy_results, overall_performance = self.calculate_comprehensive_accuracy()

        # 5. Create demographic report and visualizations (already called in create_comprehensive_demographic_report)
        # self.create_comprehensive_demographic_report()

        # 6. Create 3D structure visualizations for example sequences
        print("\n" + "="*80)
        print("GENERATING EXAMPLE 3D DNA STRUCTURE VISUALIZATIONS")
        print("="*80)

        # Select example sequences
        example_sequences = self.data.sample(min(3, len(self.data)), random_state=42)

        for index, row in example_sequences.iterrows():
            sequence = row['DNA_Sequence']
            structure = row['Structure_3D']
            defect = row['Structural_Defect'] == 'Present'
            print(f"\nVisualizing Sequence ID: {row['ID']}")
            print(f"  Sequence: {sequence}")
            print(f"  Predicted Structure: {structure}")
            print(f"  Predicted Defect: {'Present' if defect else 'Absent'}")

            self.create_enhanced_double_helix(sequence, structure_type=structure, defect_present=defect)

        print("\nDNA Deep Learning Analysis Pipeline Completed.")
        print("System Performance Metrics:")
        print(f"  Best Structure Model Accuracy: {overall_performance['system_overall']['best_structure_accuracy']:.4f}")
        print(f"  Best Defect Model Accuracy: {overall_performance['system_overall']['best_defect_accuracy']:.4f}")
        print(f"  Combined System Accuracy: {overall_performance['system_overall']['combined_accuracy']:.4f}")
...
if __name__ == "__main__":
    # Example Usage (optional - uncomment to run the full pipeline)
    analyzer = DNADeepLearningAnalyzer()
